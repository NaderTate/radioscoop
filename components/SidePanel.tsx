...
const data = [
  ...
  //   link: "/radio-ac/6339d983eb8c933b9fd29f1b",
  // },
  // {
...
const SidePanel = () => {
  ...
  return (
    <div className="mt-10 mb-4">
      <div className="hidden lg:flex lg:flex-col justify-center">
        {data.map((item, index) => (
          <div
            ...
            className="rounded-lg m-6 h-fit sm:h-auto sm:m-2 sm:w-72 flex flex-col justify-between p-2 shadow-xl bg-slate-700 hover:scale-[1.01] cursor-pointer"
          ...
          >
            <div className="w-full rounded-lg p-3 shadow-sm ">
              <img
                alt={item.title}
                src={item.image}
                className=" object-contain w-fit h-fit rounded-lg "
              />
            </div>

            <blockquote className="flex flex-col justify-between  p-1 text-center rounded-lg ">
      ...
      >
        {data.map((item, index) => (
          <SwiperSlide key={index + item.title} className="flex justify-center">
            <div
              style={{
              ...
              className="rounded-lg h-80 m-6 sm:m-2 w-72 justify-end flex flex-col  p-2 shadow-xl  hover:scale-[1.01] cursor-pointer"
              onClick={() => handleRoute(item.link)}
            >
              {item.title.length > 1 && (
